...
impl Sbox {
    ...
    pub fn generate_ddt(&self, save_dir: &str){
        let mut ddt: Vec<Vec<i16>> = vec![vec![0; 256]; 256];
        let save_path = format!("{}/DDT_{}.csv", save_dir, self.name);
        ...
        match Self::save_csv(&ddt, &save_path) {
            Ok(_) => println!("DDT of {} save in {}", self.name, save_path),
            Err(e) => eprint!("Error saving DDT of {}: {}", self.name, e),
        };
    }

    pub fn generate_lat(&self, save_dir: &str){
        let mut lat: Vec<Vec<i16>> = vec![vec![0; 256]; 256];
        let save_path = format!("{}/LAT_{}.csv", save_dir, self.name);
        ...
        match Self::save_csv(&lat, &save_path) {
            Ok(_) => println!("DDT of {} save in {}", self.name, save_path),
            Err(e) => eprint!("Error saving LAT of {}: {}", self.name, e),
        };
    }
    ...
    fn save_csv(ddt: &Vec<Vec<i16>>, save_path: &str) -> Result<(), Box<dyn Error>> {
        ...
        for row in ddt {
            wtr.write_record(row.iter().map(|&val| val.to_string()))?;
        }
